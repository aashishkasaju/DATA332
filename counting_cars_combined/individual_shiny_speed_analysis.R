...
# Modified: Clean and standardize the CSV dataset (Tanner's group)
csv_data_clean <- csv_data %>%
  transmute(
    Initial_Speed = as.numeric(Initial_Read),
    Final_Speed = as.numeric(Final_Read),
    Difference = as.numeric(Difference_In_Readings),
    Body_Style = recode(as.character(Type_of_Car),
                        "1" = "Emergency",
                        "2" = "Hatchback",
                        "3" = "Sedan",
                        "4" = "SUV",
...
# Modified: Clean and standardize the Excel dataset (Basil's group)
excel_data_clean <- excel_data %>%
  transmute(
    Initial_Speed = as.numeric(init_speed),
    Final_Speed = as.numeric(final_speed),
    Difference = as.numeric(speed_change),
    Body_Style = vehicle_type
  ) %>%
  mutate(
    Body_Style = case_when(
      Body_Style %in% c("Hatchback", "Coupe", "Sedan") ~ "Sedan",
      Body_Style %in% c("Van", "Minivan", "SUV") ~ "SUV",
      Body_Style %in% c("Truck", "Pickup Truck") ~ "Truck",
      TRUE ~ Body_Style
    )
  )
